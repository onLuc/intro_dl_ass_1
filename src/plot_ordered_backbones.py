...
need = [c for c in ["model", "epoch", "time_s", "val_cse_mean", "backbone"] if c in df.columns]
missing = {"val_cse_mean", "backbone"} - set(need)
if "val_cse_mean" not in need:
    raise ValueError("epoch_stats_long.csv must contain a 'val_cse_mean' column.")
df = df[need].copy()
...
if "time_s" in df.columns and df["time_s"].notna().any():
    g_time = _ordered_group_mean(df, "time_s")
    plt.figure(figsize=(6.2, 4.2))
    xs = np.arange(len(g_time))
    bar_colors = [BB_COLORS[str(b)] for b in g_time["backbone"]]
    plt.bar(xs, g_time["time_s"], color=bar_colors, edgecolor="#222222", linewidth=0.6)
    plt.xticks(xs, [str(b).capitalize() for b in g_time["backbone"]])
    plt.ylabel("Avg. Time per Epoch (s)")
    plt.title("Average Training Time per Epoch by Backbone")
    for x, y in zip(xs, g_time["time_s"]):
        plt.text(x, y, f"{y:.1f}", ha="center", va="bottom", fontsize=8)
    plt.tight_layout()
