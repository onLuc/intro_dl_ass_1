...
need = [c for c in ["model", "epoch", "time_s", "val_cse_mean", "type", "backbone"] if c in df.columns]
if "val_cse_mean" not in need or "epoch" not in need:
...
def infer_type_from_model(name: str) -> str:
    ...
    if "classifier" in n: return "classification"
    return "other"
...
agg = (
    df.dropna(subset=["val_cse_mean"])
      .groupby(["type", "backbone", "epoch"], as_index=False)["val_cse_mean"]
      .mean()
)
